...
impl ResponseCache {
    ...
    pub async fn put(&self, query: &str, response: CachedResponse) {
        self.put_with_ttl(query, response, self.config.default_ttl).await;
    }
    ...
    fn calculate_similarity(&self, query1: &str, query2: &str) -> f32 {
        ...
        let words1: std::collections::HashSet<_> = lower1
            .split_whitespace()
            .collect();
        
        let words2: std::collections::HashSet<_> = lower2
            .split_whitespace()
            .collect();
...
mod tests {
    ...
    async fn test_tag_invalidation() {
        ...
        cache.put_with_tags("test1", response.clone(), vec!["group1".to_string()]).await;
        cache.put_with_tags("test2", response, vec!["group1".to_string()]).await;
