...
impl SecuritySandbox {
    ...
    pub async fn check_network(&self, domain: &str) -> SecurityCheckResult {
        ...
        if !self.config.network_rules.allow_any {
            let in_allowed = self.config.network_rules.allowed_domains.iter()
    ...
    pub async fn check_command(&self, command: &str) -> SecurityCheckResult {
        ...
        if self.config.command_rules.whitelist_mode {
            let in_allowed = self.config.command_rules.allowed.iter()
...
mod tests {
    ...
    async fn test_path_check() {
        ...
        let result = sandbox.check_path(std::path::Path::new("/etc/passwd"), false).await;
        assert!(!result.allowed);
        
        // Normal path should be allowed
        let result = sandbox.check_path(std::path::Path::new("/tmp/test.txt"), false).await;
        assert!(result.allowed);
    }
    ...
    async fn test_file_modification_limit() {
        ...
        sandbox.record_file_modified(std::path::Path::new("/tmp/file1.txt")).await;
        sandbox.record_file_modified(std::path::Path::new("/tmp/file2.txt")).await;
        
        // Third file should be denied
        let result = sandbox.check_path(std::path::Path::new("/tmp/file3.txt"), true).await;
        assert!(!result.allowed);
