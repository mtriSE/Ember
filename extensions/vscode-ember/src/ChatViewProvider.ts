...
export class ChatViewProvider implements vscode.WebviewViewProvider {
    public static readonly viewType = 'ember.chatView';
    private _view?: vscode.WebviewView;
    private _messages: Array<{ role: string; content: string }> = [];

    constructor(
        private readonly _extensionUri: vscode.Uri,
        private readonly _client: EmberClient
    ) {}
    ...
    private async _handleMessage(message: string) {
        ...
        this._client.chatStream(
            message,
            { messages: this._messages.map(m => ({ role: m.role as 'user' | 'assistant', content: m.content })) },
            (chunk) => {
                fullResponse += chunk;
